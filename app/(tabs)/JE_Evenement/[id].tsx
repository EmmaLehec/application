 ...
 const [inscrit, setInscrit] = useState<any[]>([]);  
  ...
  useEffect(() => {
    ...
    fetch(URL_inscrire)
      ...
      .catch(err => setError(err.message)); // Gestion des erreurs (réseau, etc.)
  }, [evenement]); // Se relance uniquement si l'ID change
  ...
  const handlePlace = () => {
  const liste_inscrit = utilisateur.map((u:any) => `•${u.Prenom_uti} ${u.Nom_uti}`).join('\n');

  Alert.alert(
    'Intervenants inscrits :',
    liste_inscrit || 'Aucun intervenant pour le moment'
  );
};
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.titreWrapper}>
        <TouchableOpacity onPress={() => router.push('../JE_Evenements')} style={styles.backButton}>
         ...
         <ImageBackground
          ...
          <View style={styles.cadre_eve}>
            ...
            <Text style={[styles.titre,{marginBottom: 30}]}>Crée par {admin?.prenom_admin} {admin?.nom_admin} </Text>
              ...
              <TouchableOpacity style={styles.cadre_place}  onPress={handlePlace}>
